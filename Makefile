...
composer:
	...
	@docker-compose run --rm php composer update --quiet --no-cache --prefer-$(DEPENDENCY_VERSIONS) --prefer-stable 2>/dev/null
...
phpunit: composer
	@docker-compose run --rm -eXDEBUG_MODE=debug php php vendor/bin/phpunit --color=always 2>/dev/null
